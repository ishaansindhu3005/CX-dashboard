...
Return approvals require a ₹100 coupon (no approval needed) or a ₹250 coupon/exception return (supervisor approval needed). Today, this approval happens in a WhatsApp group, meaning:
- No record of who approved what and when.
- Supervisors cannot see the volume of approvals being requested.
- Junior agents do not always know who to escalate to, or whether an escalation is needed.

### 3. Warehouse-CX Coordination Is Manual

When a warehouse short-picks an order (cannot fulfil an item), there is no structured handoff to CX. The warehouse team messages on WhatsApp, CX calls the customer, but none of this is logged. The customer may not get a call, or may get multiple calls.

### 4. No Unified Customer View
...
**What it does:**
Controls who can log in and what they can see and do.

**User-facing behaviour:**
- A login screen shows a dropdown of all CX team members. The user selects their name and is signed in.
...
- Only Warehouse Users can send a return to Pidge.
- Only Admins can create or deactivate other users.
- Only Admins can override the refund amount when creating a manual refund.
- Agents can initiate returns from the Orders module and submit return forms, but cannot approve their own submissions.
...
**What it does:**
Gives each user a personalised operational view when they open the app.
...
Same four KPI cards but scoped to the agent's own workload. Below that, three side-by-side work queues:
- **My Returns** — up to 5 returns assigned to this agent needing action. Each has an "Open" button that deep-links directly to that return.
...
**What it does:**
A unified order lookup tool. Agents can search any order, see all its details, and initiate a return directly from here.

**Order list:**
- Search by order ID or customer phone number.
- Filter by order status (delivered, cancelled, failed, etc.), store, and whether the order has a return.
- Sortable by date (newest/oldest) and order amount.
- Shows 50 orders per page.
- Table columns: Order ID, Phone, Amount, Status, Store, Return flag, Date, Coupon code.

**Order detail panel (click any row):**

Header bar: Order number, phone, store, date, amount, status badge.

**Order Info card** (left):
- Coupon code and discount amount applied.
- Customer ID, store name.
- Tags: New / Repeat customer, T&B (Try & Buy) flag.

**CX Actions card** (right):
- If a return already exists in the system → shows the return status badge and a link to the Returns module.
- If the order is delivered and no return exists → shows a "↩ Return Item(s)" button.
- If the order is not delivered → shows a message that returns are only for delivered orders.

**Return initiation form** (appears full-width below the info row):
If the agent clicks "Return Item(s)", a form opens allowing them to select which items from the order to return, specify return vs. exchange, payment method (prepaid vs. COD), the reason, pickup date/time slot, and any notes. Submitting this creates a return in the Returns module at the "Pending Approval" stage.

**Items table** (below, grouped by store):
Shows all items in the order with columns: item name + brand (with return/Rx badge if applicable), SKU, quantity, MRP price, selling price, and discount.

**Order Summary card** (right-aligned, below items):
...
**What it does:**
A 360° customer profile tool. Agents can look up any customer by phone or user ID and see their full history across orders, returns, calls, and coupons.

**Customer list:**
- Search by phone or user ID.
- Table: User ID, Phone, Total Orders, Returns, Total Spent, First Order Date, Customer Type (New/Repeat).
- 30 customers per page.

**Customer profile panel (click any row):**
...
**Coupons Used:** All unique coupon codes this customer has used, shown as pills.

**Order History:** Last 10 orders (expandable to full history) with Order ID, Date, Amount, Status, Store, Return flag.

**Returns in System:** All returns logged against this customer's phone — with return ID, status, type (return/exchange), and date.

**CRM Calls:** All retention calls logged against this customer — with order ID, call status, and the drop-off reason noted by the agent.
...
**What it does:**
The core operational hub for the returns and exchange lifecycle. Every return flows through a 5-stage process, each stage owned by a different role.
...
| Pending Pickup | Approved, waiting for warehouse to assign to Pidge | Warehouse User |
| Out for Pickup | Rider is en route to collect the item | No action needed |
| Completed | Item collected, refund triggered | — |
| Cancelled | Rejected by CX Lead | — |

**Tabs:** One tab per stage, each showing a count of returns in that stage. An "All" tab shows everything.
...
**Summary metrics above the table:** Total count, total return value, count created today, and a returns vs. exchanges breakdown.
...
- Refund source — for prepaid: wallet credit or original source refund; for COD: wallet only; hidden if exchange.
- Pickup date and preferred time slot (7 slots available, minimum today).
- Internal notes.

Submitting moves the return to **Pending Approval**.

**CX Lead (Pending Approval):**
Sees all form details submitted by the agent (read-only). Can either:
- ✓ Approve → moves to **Pending Pickup**.
- ✗ Reject → must provide a written reason → moves to **Cancelled**.

Agents see a "Submitted — awaiting CX Lead approval" message on this return and cannot edit it.

**Warehouse User (Pending Pickup):**
Sees all approved details. Selects the store and clicks "Send to Pidge ↗". System generates a Pidge tracking ID and moves the return to **Out for Pickup**.

**Out for Pickup (all roles):**
Shows the Pidge tracking ID. No action needed. Admin/Supervisor can simulate Pidge completion in v1 (real integration in v2), which marks the return **Completed** and auto-triggers a refund.

**Manual Return Creation (CX Lead and above):**
A "Create Return" button at the top right allows CX Leads to manually create a return for an order — entering the order ID, customer details, and all fields — without the agent submission step. This is used for edge cases (expired return window, non-app returns, admin-initiated refunds). Submits directly to **Pending Approval**.
...
**What it does:**
Tracks every refund from creation to completion, with a 3-step approval and processing flow. Refunds can be auto-created (triggered when a return completes) or manually created by CX Leads.

**Refund stages:**

| Stage | What's happening | Who acts |
|-------|-----------------|----------|
| Pending Approval | Refund created, waiting for CX Lead to approve | CX Lead / Supervisor |
| Pending | Approved, waiting to be processed by finance/system | CX Lead / Supervisor |
...
**Refund types tracked:** App return, COD partial, undelivered, manual (OOS, exception).

**Refund methods tracked:** Wallet credit, Source refund (original payment method), COD wallet.
...
**Filters:** Customer phone/name, refund method, refund type, date range.

**Summary metrics:** Total refund count, total refund amount.

**Table columns:** Refund ID, Order ID, Customer, Order Amount, Refund Amount, Method, Type, Linked Return, Status.
...
Enter an Order ID → system auto-fills customer phone, customer ID, and order amount from the CSV (read-only). The CX Lead selects refund method, type, and adds notes. Admin can change the refund amount. Submits to **Pending Approval**.
...
**What it does:**
A structured retention call management system. When orders fail (RTO, cancelled, undelivered, failed payment), CX agents call customers to re-engage them. Every call is logged with outcome and reason.

**Why this exists:**
Without this, agents pick up a spreadsheet or WhatsApp list and make calls with no standard process, no logging, and no visibility into whether a customer was reached or what happened.
...
**Auto-assignment:**
When an agent marks themselves Available in the sidebar, unassigned pending calls are automatically distributed to them using round-robin logic. No supervisor needs to manually assign.

**Filters (Pending tab):**
- Order status filter (RTO / Cancelled / Failed / etc.)
...
**Detail panel (what the agent sees):**
...
Left column (the form the agent fills in):
- **Reached out:** Yes / No / Attempted.
- **Drop-off reason:** Why the order failed or customer didn't reorder (CX unavailable, not interested, price too expensive, bad delivery experience, forgot coupon, too slow, product quality, other).
- **Reordered:** Yes / No.
- **Notes:** Free text.
...
**What it does:**
When a warehouse cannot fulfil one or more items in an order (short pick), a task is created for a CX agent to call the customer, explain the situation, and offer a resolution (refund, replacement, or cancellation).

**Why this exists:**
Currently, warehouse staff message CX on WhatsApp when a short pick happens. There is no tracking of whether the customer was called, what was offered, or how it was resolved. Short picks can go unaddressed.
...
**Sync with OMS:**
In v1, a sync function stub exists. In production, short-pick records will auto-flow from the OMS when a warehouse confirms a short-pick.

**Filters (Pending tab):** Store code, date range.
...
**What it does:**
Allows the Admin to manage the CX team roster inside the dashboard itself.

**User list:**
All active team members with their role badge, email, and phone.

**Create user:**
Admin fills in name, email, phone, and selects a role from the dropdown. User is immediately active and can log in.

**Edit user:**
Inline — Admin can update name, email, phone, and role for any user.

**Deactivate user:**
Soft delete — marks the user inactive (they disappear from the team list and cannot log in). Admins cannot deactivate themselves.
...
Target: Reduce average resolution time by 40% within 60 days of go-live.

### Primary Success Metrics

| Metric | Target |
|--------|--------|
| % of returns with full audit trail | 100% (vs. 0% today — all done on WhatsApp) |
| % of retention calls logged with outcome | 100% (vs. ~0% today — no logging system) |
...
### Availability
- Dashboard must be accessible during all CX shift hours (8 AM – 10 PM IST).
- Acceptable downtime: <30 minutes per week.
- Data must not be lost on restart (SQLite WAL mode ensures durability; future migration to MySQL for production-grade persistence).

### Performance
- Order and Customer list pages: Load <3 seconds for 50,000 rows (CSV cached in memory).
- Return detail panel: Open and render <1 second.
- Return status update (approve/reject): Reflect in UI within 500ms.
...
### Scalability
- Phase 1 (v1): CSV-powered. Works for the current team of 6–10 agents and ~50,000 historical orders.
- Phase 2: Replace CSV with live API/DB reads. Replace SQLite with MySQL. No structural code changes needed — only the data layer changes.

---

## Analytics & Events

The following operational events should be tracked (for future reporting and SLA monitoring):
...
| ID | Scenario | Expected Behaviour |
|----|----------|--------------------|
| DASH-01 | Agent logs in with 3 pending returns, 2 calls | Dashboard shows 3 My Returns queue items, 2 My CRM Calls items, 0 My Short Picks. |
...
| ORD-05 | Agent opens an order that already has a return in the system | CX Actions card shows the return status badge and a link to the Returns module. |
...
| ORD-09 | COD order opened | Return form shows only "Wallet" as refund source option (source refund not available for COD). |
...
| RET-04 | Agent submits COD order return | Refund Source options show only "Wallet" (no source refund). |
...
| RET-09 | Return reaches Completed | If type = return: a refund record is automatically created in the Refunds module at Pending Approval. |
| RET-10 | Return reaches Completed | If type = exchange: no refund is auto-created. |
| RET-11 | Pending Action tab count | Badge count on tab reflects actual number of pending_action returns. |
...
| ID | Scenario | Expected Behaviour |
|----|----------|--------------------|
| CRM-01 | Agent marks Available with unassigned calls | Pending calls distributed to agent via round-robin. Count visible on Dashboard. |
...
| USR-03 | Admin deactivates a user | User disappears from login dropdown and team queue. Existing records they created are preserved. |
...
**Audience:** Ishaan + 1–2 trusted agents (Bhagwana, Priya)
...
**Audience:** All CX agents and warehouse staff.
...
| Coupon validation in-dashboard | Not built | Create and validate ₹100/₹250 coupons directly in dashboard (currently done in Admin Panel) |
| CX performance reports | Not built | Tabular + chart view of agent resolution times, approval rates, call outcomes by week |
| Shift scheduling | Not built | Calendar-based shift planner (Google Calendar-style) for team leads |
